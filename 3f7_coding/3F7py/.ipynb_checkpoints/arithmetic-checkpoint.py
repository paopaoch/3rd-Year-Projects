...
def encode(x, p):
    ...
    for k in tqdm(range(len(x))): # for every symbol
        ...
        while True:
            if hi < half: # if lo < hi < 1/2
                ...
                y.extend([1 for i in range(straddle)]) # extend by a sequence of 'straddle' ones
                ...
                y.extend([0 for i in range(straddle)]) # extend 'straddle' zeros
    ...
    if lo < quarter: # the position of lo determines the dyadic interval that fits
        ...
        y.append(0)
        y.extend([1 for i in range(straddle)])

    else:
        # output a 1 followed by "straddle" zeros
        y.append(1)
        y.extend([0 for i in range(straddle)])
